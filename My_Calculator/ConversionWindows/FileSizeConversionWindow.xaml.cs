        ...
        private void CalculateConversion()
        {
            if (IsLoaded && UnitComboBox.SelectedItem is ComboBoxItem item)
            {
                if (double.TryParse(MainOutputText.Text, out double currentValue))
                {
                    ...
                    try
                    {
                        FileSizeTypeEnum currentTimeType = (FileSizeTypeEnum)Enum.Parse(typeof(FileSizeTypeEnum), currentFileSizeTypeString, true);
                        FileSizeConversion.SetValueToConvert(currentValue, currentTimeType);
