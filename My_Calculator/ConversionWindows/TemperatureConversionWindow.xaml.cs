...
{
    /// <summary>
    /// Interaction logic for TemperatureConversionWindow.xaml
    /// </summary>
    public partial class TemperatureConversionWindow : Window
    {
        private TimeConversion TimeConversion { get; } = new TimeConversion(0, TimeTypeEnum.Seconds);
        private TemperatureConversion TemperatureConversion { get; } = new TemperatureConversion(0, TemperatureTypeEnum.Celsius);
        ...
        private void CalculateConversion()
        {
            if (IsLoaded && UnitComboBox.SelectedItem is ComboBoxItem item)
            {
                ...
                {
                    string currentTemperatureTypeString = item.Tag as string;
                    try
                    {
                        TemperatureTypeEnum currentTemperatureType = (TemperatureTypeEnum)Enum.Parse(typeof(TemperatureTypeEnum), currentTemperatureTypeString, true);
                        TemperatureConversion.SetValueToConvert(currentValue, currentTemperatureType);
                        CelsiusCell.Content = TemperatureConversion.Celsius;
                        FahrenheitCell.Content = TemperatureConversion.Fahrenheit;
                    }
